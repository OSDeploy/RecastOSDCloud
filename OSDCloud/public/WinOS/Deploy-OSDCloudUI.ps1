function Deploy-OSDCloudUI {
    ...
    .DESCRIPTION
        Initializes and runs an OSDCloud deployment workflow. By default, launches the Fluent
        Design graphical UX (powered by WPF UI / lepoco/wpfui) so the operator can configure
        deployment settings before starting. Use -CLI to skip the UI and immediately begin the
        workflow in the current console session.

        This function is identical in behavior to Deploy-OSDCloud but defaults to the 'fluent'
        workflow channel which provides a modern Fluent Design interface. The '-CLI' switch and
        '-WorkflowName' parameter work the same way as in Deploy-OSDCloud.
    ...
    .PARAMETER WorkflowName
        The name of the OSDCloud workflow to run. Defaults to 'fluent'.
